...
router.route("/")
  .get(async (req, res) => {
    ...
    try {
      ...
      res.json({ success: true, user: { id: savedUser._id, email: savedUser.email, role: savedUser.role } });
    }
    catch (error) {
...
userPlaylistsRouter.route("/")
  .get((req, res) => {
    ...
    User.findById(userId).populate({
      ...
      populate: {
        path: "videos",
        select: { "_id": 1, "title": 1, "category": 1, "thumbnail": 1, "video": 1, "description": 1, "runtime": 1 }
      }
    }).exec((error, user) => {
      if (error) {
        console.error(error);
...
userPlaylistsRouter.route("/create")
  .post(async (req, res) => {
    try {
...
userPlaylistsRouter.route("/delete")
  .post(async (req, res) => {
    try {
      let { user } = req;
      ...
      res.json({ success: true, message: `Playlist ${playlist.title} deleted`, playlist })
    }
    catch (error) {
      res.status(500).json({ success: false, message: "error while deleting playlist", errorMessage: error.message })
    }
  })


userCategorySubscriptionsRouter.route("/")
  .get(async (req, res) => {
    const { userId } = req.params;
    User.findById(userId).populate("categorySubscriptions").exec((error, user) => {
      if (error) {
        console.error(error);
        return res.status(500).json({ success: false, message: "Error while retreiving user catgories", errorMessage: error.message });
      }
      return res.json({ success: true, categorySubscriptions: user.categorySubscriptions })
    });
  })


userCategorySubscriptionsRouter.route("/sub")
  .post(async (req, res) => {
    try {
      ...
      res.json({ success: true, message: `User subbed to ${category.name}`, category })
    }
    catch (error) {
      res.status(500).json({ success: false, message: "Error occurred while subscribing", errorMessage: error.message });
    }
  })


userCategorySubscriptionsRouter.route("/unsub")
  .post(async (req, res) => {
    try {
      let { user } = req;
      const { categoryId } = req.body;
      user.categorySubscriptions = user.categorySubscriptions.filter(item => String(item) !== categoryId);
      user = await user.save();
      let category = await Category.findById(categoryId);
      res.json({ success: true, message: `User unsubbed from ${category.name}`, category })
    }
    catch (error) {
      res.status(500).json({ success: false, message: "error while unsubbing", errorMessage: error.message })
    }
  })
