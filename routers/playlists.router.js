...
router.param("playlistId", async (req, res, next, playlistId) => {
  try {
    ...
    return res
      .status(400)
      .json({ success: false, message: "Error while retreiving the playlist" });
  }
});
...
playlistVideoRouter.route("/add").post(async (req, res) => {
  try {
...
playlistVideoRouter.route("/delete").post(async (req, res) => {
  try {
